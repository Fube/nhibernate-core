...
public sealed class IdentityMap : IDictionary, IDeserializationCallback
{
	...
	{
		return ((IdentityMap)map).GetEntries();
	}
...
/// <summary>
/// A read-only view of a snapshot-capable dictionary, frozen at the moment the snapshot was
/// taken. Dispose it (e.g. via <c>using</c>) as soon as you are done reading it, so the owning
/// dictionary can go back to fully in-place, allocation-free mutation once every outstanding
...
[Serializable]
internal sealed class IdentityMap<TKey, TValue> : IDictionary<TKey, TValue>, IDeserializationCallback
	where TKey : class
	where TValue : class
{
	// key = IdentityKey of the passed in Key
	...
	internal static IdentityMap<TKey, TValue> Instantiate(int size) =>
	...
	/// </remarks>
	internal static IdentityMap<TKey, TValue> InstantiateSequenced(int size) =>
	...
	/// </summary>
	/// <param name="underlyingMap">A class that implements the IDictionary for storing the objects.</param>
	private IdentityMap(IDictionary<TKey, TValue> underlyingMap)
	{
		_map = underlyingMap;
	}
	...
	{
		if (_map is SequencedSnapshotDictionary<TKey, TValue> snapshotDictionary)
		{
			return snapshotDictionary.GetSnapshot();
		}

		return new SnapshotViewAdapter(_map.ToList());
	}
	...
	/// <returns>An object that is safe to be a key.</returns>
	/// <exception cref="ArgumentException">Thrown when the obj is a System.ValueType</exception>
	private static void VerifyValidKey(TKey obj)
	{
		if (obj is ValueType)
		{
			throw new ArgumentException(
	...
	internal static IDictionary<TValue, TKey> Invert(IDictionary<TKey, TValue> map)
	{
		IDictionary<TValue, TKey> result = IdentityMap<TValue, TKey>.Instantiate(map.Count);
	...
	{
		((IDeserializationCallback)_map).OnDeserialization(sender);
	}
	
	private sealed class SnapshotViewAdapter : ISnapshotView<TKey, TValue>
	{
