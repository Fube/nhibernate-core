...
namespace NHibernate.Engine
{
	...
	public partial class StatefulPersistenceContext : IPersistenceContext, ISerializable, IDeserializationCallback
	{
		...
		private readonly IDictionary<object, object> entityEntries;
		...
		private readonly IDictionary<object, object> collectionEntries;
		...
		public void Clear()
		{
			...
			var collectionEntryArray = IdentityMap<object, object>.GetEntries(collectionEntries);
		...
		{
			arrayHolders.TryGetValue(array, out var holder);
			return holder;
		}
		...
		{
			arrayHolders.TryGetValue(array, out var tempObject);
		...
		internal StatefulPersistenceContext(SerializationInfo info, StreamingContext context)
		{
			...
			entityEntries = (IdentityMap<object, object>)info.GetValue("context.entityEntries", typeof(IdentityMap));
			...
			collectionEntries = (IdentityMap<object, object>)info.GetValue("context.collectionEntries", typeof(IdentityMap));
